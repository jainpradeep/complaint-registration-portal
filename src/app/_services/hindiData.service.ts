...
export class HindiDataService {
    ...
    constructor(private http: HttpClient,private spinner: NgxSpinnerService, windowScrolling: WindowScrolling) { 
     ...
     insertRecord(hindiStatusReport:any) {
        ...
        return this.http.post<any>('http://10.14.151.91:3006/insertHindiReport', hindiStatusReport)
            .pipe(map(res => {
    ...
    getRecords(filters: any){
         this.windowScrolling.disable(); this.spinner.show(); 
        return this.http.post<any>('http://10.14.151.91:3006/getHindiData', 
        filters )
        .pipe(map(res => {
            this.spinner.hide();  this.windowScrolling.enable();  
            if (res.msg == "success") {
    ...
    deleteHindiReport(record:any){
         this.windowScrolling.disable(); this.spinner.show(); 
        return this.http.post<any>('http://10.14.151.91:3006/deleteHindiData', record )
        .pipe(map(res => {
            this.spinner.hide();  this.windowScrolling.enable();  
            if (res.msg == "success") {
                return res;
    ...
    editHindiReport(record:any){
        ...
        return this.http.post<any>('http://10.14.151.91:3006/editHindiData', record )
        .pipe(map(res => {
            this.spinner.hide();  this.windowScrolling.enable();  
            if (res.msg == "success") {
                return res;
